...
logging.info("# - Best score r2=%f using %s" % (search.best_score_, search.best_params_))
...
if (args.dosage_dir_test is not None) and (args.dosage_fn_test is not None):
    ...
    logging.info('# Coefficient of determination score (test) r2=%.4f' % r2_score(y_test, search.predict(X_test)))
